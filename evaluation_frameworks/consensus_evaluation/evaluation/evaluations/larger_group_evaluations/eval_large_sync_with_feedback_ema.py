...
def _compute_one_group(args):
    ...
    factory_method = lambda single_user_model, evaluation_set_csr: (
        SyncMediatorFactoryBuilderSync()
        .with_group_algorithm(lambda: GR_AggregatedProfilesUpdatable(single_user_model, update_mode="ema"))
...
class EvalLargeSyncWithFeedbackEma(ConsensusExperimentBase):
    ...
    def make_table(self, results: Dict[str, Dict[str, Any]]) -> str:
        ...
        cap = (
            ...
            rf"sync s EMA zpětnou vazbou, agg={self.agg_strategy!r}, $w={self.w_size}$."
        )
